...
#include <linux/fb.h>
#include <linux/proc_fs.h>   //proc file use by  liunianliang
#ifdef CONFIG_COMPAT
...
//add by liunianliang for hardware info 20160128 begin
static struct tpd_driver_t *g_tpd_drv;
#define PROC_CTP_INFO "driver/ctp_info"
static int ctp_info_read(struct seq_file *m, void *v)
{
   seq_printf(m, "%s\n",g_tpd_drv->tpd_device_name);
   return 0;
};
static int proc_ctp_info_open(struct inode *inode, struct file *file)
{
    return single_open(file, ctp_info_read, NULL);
};

static  struct file_operations fctp_proc_fops1 = {
    .owner = THIS_MODULE,
    .open  = proc_ctp_info_open,
    .read  = seq_read,
};
//add by liunianliang for hardware info 20160128 end
...
static int tpd_probe(struct platform_device *pdev)
{
	...
	proc_create(PROC_CTP_INFO, 0, NULL, &fctp_proc_fops1);   //add by liunianliang
